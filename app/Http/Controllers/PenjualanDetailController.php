...
class PenjualanDetailController extends Controller
{
    public function index()
    {
        ...
        $member = Member::orderBy('nama')->where('status','1')->get();

        // Cek apakah ada transaksi yang sedang jalann 
    ...
    public function data($id)
    {
        ...
        foreach ($detail as $item) {
            $row = array();
            $row['kode_produk'] = '<span class="label label-info">'. $item->produk['kode_produk'] .'</span';
            ...
            <button onclick="deleteData(`'. route('transaksi.destroy', $item->id_penjualan_detail) .'`)" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></button>
            </div>';
            $data[] = $row; 
